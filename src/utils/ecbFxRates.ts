...
function findSameOrNextPublishedRate(date: string): number | undefined {
  ...
  const nextDate = sortedDates.find(candidate => candidate > date);
...
export async function getUsdEurRatesForDates(dates: string[]): Promise<Record<string, number>> {
  ...
  const missingDates = requiredDates.filter(date => !findSameOrNextPublishedRate(date));
  if (missingDates.length > 0) {
    await fetchMissingRange(missingDates[0], missingDates[missingDates.length - 1]);
  }
