...
describe('ecbFxRates', () => {
  ...
  it('fetches only missing overlapping dates and merges them into cache', async () => {
    ...
    expect(fetchMock.mock.calls[1]?.[0]).toContain('endPeriod=2025-03-05');
...
`;
  ...
  it('uses persisted published dates to satisfy later weekend lookups without refetching', async () => {
    globalThis.localStorage.setItem(
      ECB_USD_EUR_CACHE_STORAGE_KEY,
      JSON.stringify({ '2025-03-03': 1.0465 }),
    );
